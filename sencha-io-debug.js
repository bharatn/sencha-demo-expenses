...
Ext.define('Ext.io', {
	...
	// JCM Ext has a standard way of doing config... shouldn't we use that mechanism?
  ...
  statics: {
    ...
    setup: function(config) {
      Ext.io.config = config;
      if(config.logLevel) {
        Ext.util.Logger.setLevel(config.logLevel);
      }
    },
    ...
    init: function() {
	  ...
	  */
    getCurrentApp: function(callback) {
      Ext.io.init();
      Ext.io.App.getCurrent(callback);
    },
	  ...
	  * @param {Function} callback
	  */
    getCurrentClient: function(callback) {
      Ext.io.init();
      Ext.io.Client.getCurrent(callback);
    },

	  /**
	  * Get the current UserGroup, if any.
	  *
	  * @param {Function} callback
	  */
    getCurrentUserGroup: function(callback) {
      Ext.io.init();
			Ext.io.UserGroup.getCurrent(callback);
    },
    ...
    getCurrentUser: function(callback) {
      Ext.io.init();
			return Ext.io.User.getCurrent(callback);
    },
    ...
    getService: function(name, callback) {
      Ext.io.init();
      Ext.io.messaging.get(name, callback);
    },
...
Ext.define('Ext.io.Proxy', {
  ...
  constructor: function(name, descriptor, rpc) {
    ...
    if(this.descriptor.kind != 'rpc') {
			Ext.util.Logger.error(this.name + " is not a RPC service. Use Ext.io.send to send an async message to the service.");
...
Ext.define('Ext.io.Messaging', {
  ...
  constructor: function(config) {
    ...
    this.realms = Ext.create('Ext.io.Entities', 'Realms', this);
    this.serviceEntities = Ext.create('Ext.io.Entities', 'ServiceEntities', this);
    this.organizations = Ext.create('Ext.io.Entities', 'Organizations', this);
  ...
  getQueue: function(queueName, callbackFunction) {
    var self = this;

    var queue = this.queueCache[queueName];
    if(queue) {
      callbackFunction(queue);
    } else {
...
Ext.define('Ext.io.PubSub', {
  ...
  handleIncoming: function(envelope) {
	console.log('handleIncoming',envelope)
...
Ext.define('Ext.io.Queue', {
  ...
  */
  publish: function(message, callbackFunction, scope) {
    this.pubsub.publish(this.name, message, callbackFunction, scope);
  },

  /**
  * Subscribe to receive messages from this queue.
  */
  subscribe: function(callbackFunction, scope, errCallbackFunction) {
    this.pubsub.subscribe(this.name, callbackFunction, scope, errCallbackFunction);
  },

  /**
  * Unsubscribe from receiving messages from this queue.
  */
  unsubscribe: function(callbackFunction, scope) {
    this.pubsub.unsubscribe(this.name, callbackFunction, scope);
  }
});
...
Ext.define('Ext.io.Naming', {
  ...
  getClientIdFromLocalStorage: function() {
    ...
    try {
      if('localStorage' in window && window['localStorage'] !== null) {
        clientId = window.localStorage["sencha.io.client.uuid"];
      }
    } catch(e) {
      clientId = null;
    }
  ...
  storeClientIdInLocalStorage: function(clientId) {
    window.localStorage["sencha.io.client.uuid"] = clientId;
  },
    ...
    if(serviceName == "naming-rpc") {
      callbackFunction({
        ...
        methods: ["getServiceDescriptor",
          "get", "find", "update",
          "getSingleLink", "getRelatedEntities", "findRelatedEntities"]
      });
    } else {
      this.messaging.get("naming-rpc", function(namingRpc) {
...
Ext.define('Ext.io.Entity', {
  ...
  getSingleLink: function(Bucket, Key, Tag, Entity, callbackFunction) {
    var self = this;
    ...
    this.messaging.get("naming-rpc", function(namingRpc) {
      namingRpc.getSingleLink(function(result) {
        if(result.status == "success") {
          ...
          callbackFunction(false, linkedEntity);
        } else {
  ...
  getRelatedEntities: function(Bucket, Tag, Entity, callbackFunction) {
    var self = this;
    ...
    this.messaging.get("naming-rpc", function(namingRpc) {
      namingRpc.getRelatedEntities(function(result) {
        if(result.status == "success") {
          ...
          callbackFunction(false, objects);
        } else {
          callbackFunction(true, null);
        }
  ...
  findRelatedEntities: function(Bucket, Key, Tag, Query, Entity, callbackFunction) {
    var self = this;
...
Ext.define('Ext.io.Entities', {
  CLASS_MAP: {
    'Realms': 'Ext.io.UserGroup',
    'Apps': 'Ext.io.App',
    'Users': 'Ext.io.User',
    'Instances': 'Ext.io.Client'
  },
  ...
  get: function(key, callbackFunction) {
    ...
    this.messaging.get("naming-rpc", function(namingRpc) {
      namingRpc.get(function(result) {
        if(result.status == "success") {
          callbackFunction(false, Ext.create(self.CLASS_MAP[self.bucket], self.bucket, result.value._key, result.value.data, self.messaging));
        } else {
          callbackFunction(true, null);
        }
      }, self.bucket, key);
  ...
  find: function(query, start, rows, callbackFunction) {
    var self = this;
...
Ext.define('Ext.io.App', {
  ...
  statics: {
    ...
    *
	  ...
	  */
      ...
      if (!appId) {
        callback({message: "No active apps."}, null);
      }
      new Ext.io.Entities("Apps", Ext.io.messaging).get(appId, callback);
    }
  },
  ...
  /**
  *  Get the UserGroup associated with this App, if any.
  *
  * @param {Function} callback 
  */
	getUserGroup: function(callback) {
	  this.getSingleLink("Realms", null, null, "Ext.io.UserGroup", callback);
	},
  ...
  * @param {Function} callback 
  */
	getQueue: function(config, callback) {
    this.messaging.getQueue(config.name, callback);
	},
  ...
  * @param {Function} callback 
  */
	getStore: function(config,callback) {
    console.log("JCM return Ext.Store");
	},

  /**
  * Find clients that match a query.
  * (JCM describe query)
  * @param {Object} query 
  * @param {Function} callback 
  * @returns Array - an array of Ext.io.Client
  */
  findClients: function(query, callback) {
    this.findRelatedEntities("Apps", this.key, null, query, "Ext.io.Client", function(err, clients) {
      if (err) {
        callback(err, null);
      } else {
  ...
  *
  * @returns Array - an array of Store objects.
  */
...
Ext.define('Ext.io.Client', {
  ...
  statics: {
	  ...
	  * 
	  *  @returns Ext.io.Client
	  */
    getCurrent: function(callback) {
      var clientId = Ext.io.messaging.naming.getClientId();  // JCM maybe the code in there moves here...
      new Ext.io.Entities("Instances", Ext.io.messaging).get(clientId, callback);
    }
  },
  ...
  /**
  *  Get the App associated with this Client.
  * 
  *  @returns Ext.io.App
  */
  getApp: function(callback) {
    this.getSingleLink("Versions", this.data.version, null, "Ext.io.Entity", function(err, version) {
      if (err) {
        ...
        version.getSingleLink("Apps", null, "Ext.io.App", callback);
      }
    });
  },

  /**
  *  Get the User associated with this Client, if any.
  * 
  *  @returns Ext.io.User
  */
  getUser: function(callback) {
		console.log("JCM return Ext.oi.User - not implemented on server yet");
    this.getSingleLink("Users", null, null, "Ext.io.User", callback);
	},
  ...
  */
  send: function(message) {
		console.log("JCM send message")
	},

	// Ext.io.Client.getStore(config); // => Ext.Store
...
Ext.define('Ext.io.User', {
  ...
  *
  *  @returns Array - an array of Ext.io.Client objects
  */
  getClients : function(callback) {
    this.getRelatedEntities("Instances", null, "Ext.io.Client", callback);
  },

  /**
  *  Get the UserGroup associated with this User.
  * 
  *  @returns Ext.io.UserGroup
  */
  getUserGroup : function(callback) {
    this.getSingleLink("Realms", this.data.realm, null, "Ext.io.UserGroup", callback);
  },
  ...
  */
  ...
  statics : {
    ...
    getCurrent: function(callback) {
      ...
      new Ext.io.Entities("Users", Ext.io.messaging).get(userId, callback);
    },
    ...
    getLocalStores : function() {
	  ...
	  */
	  send: function(message) {
			console.log("JCM send message, to all clients or to a user queue")
		},
 ...
 * UserGroup
 */
Ext.define('Ext.io.UserGroup', {
  extend: 'Ext.io.Entity',

  statics: {
	
		// JCM can we move the auth implementation code into another
		// JCM class, so that this interface class is easier to read,
		// JCM document?
		
    nc: 0, // request counter used in Digest auth

    getRealmId: function() {
      Ext.util.Cookie.getItem("usergroup.id");
    },

    getRequestCounter: function() {
      return ++Ext.io.UserGroup.nc;
    },

	  /**
	  *  Get the current UserGroup object.
	  * 
	  *  @returns Ext.io.UserGroup
	  */
    getCurrent: function(callback) {
      Ext.io.User.getCurrent(function(err, user) {
        if (!err) {
          user.getUserGroup(callback);
        } else {
          callback(err, null);
        }
      });
        ...
        realm.messaging.get('authorization', function(authService) {
          authService.realmAuthDigest(function(result) {
            if(result.status == "success") {
              ...
              authService.realmAuthDigest(function(result) {
                if(result.status == "success") {
                  callback(false, Ext.create('Ext.io.User', result.value._bucket, result.value._key, result.value.data, realm.messaging));
                } else {
                  callback(true, null);
                }
  ...
  */
  findUsers: function(query, callback) {
    this.findRelatedEntities("Users", this.key, null, query, 'Ext.io.User', callback);
  },
  ...
  *  @returns Ext.io.User
  */
  register: function(username, password, email, callback) { // JCM should be attributes?
    var self = this;

    this.messaging.get('authorization', function(authService) {
      authService.realmRegister(function(result) {
        if(result.status == "success") {
          callback(false, Ext.create('Ext.io.User', result.value._bucket, result.value._key, result.value.data, self.messaging));
        } else {
          callback(true, null);
        }
      }, self.key, username, password, email);
    });
  },
  ...
  */
  authenticate: function(username, password, callback) {
    Ext.io.UserGroup.authStrategies['digest'](this, username, password, callback);
  },

  /**
  *  Get the App associated with this UserGroup.
  * 
  *  @returns Ext.io.App
  */
  getApp: function(key, callback) {
    this.getSingleLink("Apps", key, null, "Ext.io.App", callback);
  }
});
...
Ext.data.SyncDatabaseDirectory= new Ext.data.ClientDatabaseDirectory();
  ...
  sync: function(store,callback,scope) {
	...
	do_sync: function(store,callback,scope) {
		...
		if(changes.added.length>0) {
			store.fireEvent('add', this, changes.added);
		}
        ...
        store.fireEvent('add', this, createdRecords);
			}
